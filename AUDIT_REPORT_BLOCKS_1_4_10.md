...
## Block 18: Bounty-Eligible Findings (HackerOne ratings)

This section filters prior findings to those most likely to qualify for HackerOne rewards (typically Medium or higher impact). Each item includes reproduction steps suitable for a report.

### 18.1 TLS certificate verification disabled for daemon WebSocket

**Severity:** Medium

**Why likely bounty-eligible:** MITM exposure of daemon/wallet traffic can impact integrity and confidentiality without user interaction beyond normal usage.

**Evidence recap:**

- WebSocket bridge and command path set `rejectUnauthorized: false`.

**Reproduction steps (from zero):**

1. Set up a TLS-intercepting proxy (e.g., `mitmproxy`) or a local TLS-terminating WebSocket server that presents a self-signed certificate.
2. Configure your host to route the GUI’s daemon WebSocket traffic through the proxy (e.g., by modifying daemon URL or system proxy, depending on your environment).
3. Launch the GUI and trigger any action that uses the daemon WebSocket (e.g., wallet status refresh).
4. Observe the connection succeeds even with the self-signed certificate and that traffic can be read or modified.
5. (Optional) Modify a response in transit to demonstrate integrity impact.

**Impact:** MITM can read/modify daemon responses, potentially misleading UI state or interfering with wallet operations.

### 18.2 IPC network fetch allows arbitrary HTTPS/IPFS URLs (SSRF-like behavior)

**Severity:** Medium

**Why likely bounty-eligible:** A renderer compromise can force the main process to access internal services not normally reachable, enabling local network probing.

**Evidence recap:**

- `FETCH_TEXT_RESPONSE` and `FETCH_POOL_INFO` accept renderer-supplied URLs.
- `fetchJSON` only enforces protocol (`https`/`ipfs`).
- CacheManager exposes multiple URL-based fetch IPCs.

**Reproduction steps (from zero):**

1. Start the GUI in a development environment where you can access the renderer console or inject JS (e.g., devtools).
2. In the renderer console, call the IPC-exposed APIs with an internal target, for example:
   - `window.appAPI.fetchTextResponse('https://127.0.0.1:8443', '{}')`
   - or `window.cacheAPI.getContent('https://127.0.0.1:8443')`
3. Observe the main process performs the request and returns content/errors to the renderer.
4. Repeat with a private LAN IP (e.g., `https://192.168.1.1/status`) to confirm internal reachability.

**Impact:** Enables internal network probing or access to services not intended to be reachable by the renderer.

### 18.3 Renderer can trigger private key/seed exposure via IPC

**Severity:** High

**Why likely bounty-eligible:** Direct exposure of seed/private keys can enable theft of funds and is typically high impact.

**Evidence recap:**

- `OPEN_KEY_DETAIL` IPC retrieves `get_private_key` and displays `secretKey`/`seed`.

**Reproduction steps (from zero):**

1. Launch the GUI and unlock a wallet with at least one key.
2. Open the renderer devtools or inject JS into the renderer context.
3. Execute: `window.appAPI.openKeyDetail('<fingerprint>')` where `<fingerprint>` is a known key fingerprint.
4. The Key Detail dialog opens; inspect the DOM to read `secretKey` and `seed`.
